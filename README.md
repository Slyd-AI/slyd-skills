...
Windows PowerShell 用户可以把变量持久化到当前 Windows 用户：

```powershell
[Environment]::SetEnvironmentVariable("SLYD_API_BASE","https://slyd.top","User")
[Environment]::SetEnvironmentVariable("SLYD_API_KEY","你的_SLYD_API_KEY","User")
```

用户级变量只会被新进程读取。执行后请关闭并重新打开 PowerShell，再从新终端启动 Agent。只想配置当前 PowerShell 会话时使用：

```powershell
$env:SLYD_API_BASE = "https://slyd.top"
$env:SLYD_API_KEY = "你的_SLYD_API_KEY"
```

不要把 API Key 粘贴到聊天中，也不要写入 Skill 目录。
...
9. 联网搜索产生来源附录时，把它识别为预期附加页，不为强行匹配 `target_pages` 而删页或重做。
...
### 指定 6 页，为什么最终 PPTX 有 7 页

`target_pages` 表示正文目标页数。启用 `web_search_enabled=true` 且搜索到可验证来源时，SLYD 会在正文后自动追加“智能搜索来源”附录页；默认 16:9 和常规来源数量下通常追加 1 页，窄画布或来源较多时可能追加多页。例如 6 页正文加 1 页来源附录，最终 7 页是正常结果，不是渲染错误，来源附录也不计入前置扣费页数。

Agent 不应仅因最终页数大于 `target_pages` 就删除来源页、重新渲染或重复提交。如果用户要求最终总页数必须严格包含来源附录，应在提交前说明规则，并让用户选择关闭联网搜索或调整正文目标页数。
