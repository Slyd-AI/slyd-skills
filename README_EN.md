...
```text
Use $skill-installer to install slyd-ppt-skill from https://github.com/Slyd-AI/slyd-skills/tree/main/skills/slyd-ppt-skill.
```

For another Skill-capable Agent, import the directory URL above with its Skill manager. You can also clone this repository and point the Agent at `skills/slyd-ppt-skill`:
...
```text
Use $slyd-ppt-skill to turn /absolute/path/report.pdf into a 12-slide Chinese management presentation. Focus on conclusions, evidence, and action items.
```
...
| Local coding Agents without a Skill manager | Manual | Clone the repository and load `skills/slyd-ppt-skill` according to the platform documentation |
...
```text
Use $slyd-ppt-skill to create a 10-slide investor update from ./quarterly-report.docx in English.
```

```text
Use $slyd-ppt-skill to visually restore these product screenshots as an editable 8-slide Chinese deck.
```

```text
Use $slyd-ppt-skill to redesign ./sales-review.pptx and add 3 summary slides from ./notes.pdf.
```
...
Current default rules are documented in [`references/api.md`](./skills/slyd-ppt-skill/references/api.md):
...
- [Skill workflow](./skills/slyd-ppt-skill/SKILL.md)
